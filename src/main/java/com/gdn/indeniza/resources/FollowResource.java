...
public class FollowResource {
	...
	public ResponseEntity<List<Follow>> findAll(){
		List<Follow> list = service.findAll(); 
		return ResponseEntity.ok().body(list);
	}
	
	@GetMapping(value = "/{id}")
	public ResponseEntity<Follow> findById(@PathVariable Long id){
		Follow obj = service.findById(id);
		return ResponseEntity.ok().body(obj);
	}
	
	@PostMapping
	public ResponseEntity<Follow> insert(@RequestBody Follow obj){
	...
	@PutMapping(value = "/{id}")
	public ResponseEntity<Follow> update(@PathVariable Long id, @RequestBody Follow obj){
